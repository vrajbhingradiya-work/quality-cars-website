        ...
        <div
          className={`${
            path !== "/emi-calculator"
              ...
              : "h-auto pt-32 "
          } flex   justify-center items-center   bg-black/80   `}
        >
          <SlideInFromBottom sequence={2}>
            <div
              className={`${
                path === "/emi-calculator"
                  ? "  lg:w-[85vw] max-w-[70vw] h-full md:h-auto m-2  my-12 md:my-16 md:mx-auto "
                  : "lg:w-[90vw]  "
...
function SelectedCarCard({
  ...
  const [carImage, setCarImage] = React.useState(
    car?.images?.[0] ? car?.images[0]?.data?.image?.url : defaultImage
  );
        ...
        >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setCarImage(selectedCar[0].images[0]?.data?.image?.url);
            }}
          >
