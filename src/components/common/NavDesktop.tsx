...
const LinksContainer = () => {
  return (
    <motion.div className="space-y-4 p-12 pl-4 w-full grid grid-cols-1 justify-center items-end gap-8 pt-16 xl:pt-36  ">
      {LINKS.map((l, idx) => {
        return (
...
const NavLink = ({
  ...
  return (
    <motion.a
      ...
      href={href}
      className="w-full xl:w-[40%] flex justify-end text-xl md:text-xl  text-white/50 transition-colors hover:text-white "
    >
      {children}
    </motion.a>
  );
  ...
  return (
    <>
      <motion.div
        ...
        className={`fixed z-30    top-0 left-0 bg-gradient-to-r from-black via-black/85  to-transparent  `}
      />
      ...
      <motion.button
        ...
        className={`group  top-4 z-50 h-20 w-20 scale-[50%] bg-white/0 transition-all hover:bg-white/20 ${
